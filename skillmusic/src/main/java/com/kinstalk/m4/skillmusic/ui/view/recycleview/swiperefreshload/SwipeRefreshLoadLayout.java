...
public class SwipeRefreshLoadLayout extends ViewGroup {
    ...
    public SwipeRefreshLoadLayout(Context context, AttributeSet attrs) {
        ...
        mFooterViewHeight = mCircleHeight * 2;
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
        // 将新添加的View,放到最后绘制
        if (mCircleViewIndex < 0 && mFooterViewIndex < 0) {
            return i;
        }
        if (i == childCount - 2) {
        ...
        int bigIndex = mFooterViewIndex > mCircleViewIndex ? mFooterViewIndex
                : mCircleViewIndex;
        int smallIndex = mFooterViewIndex < mCircleViewIndex ? mFooterViewIndex
                : mCircleViewIndex;
    ...
    private void createLoadMoreView() {
        ...
        mFooterViewContainer.setBackgroundColor(Color.TRANSPARENT);
        ...
        mFooterViewContainer.addView(loadingView, layoutParams);
    ...
    private void ensureTarget() {
        ...
        if (mTarget == null) {
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (!child.equals(mCircleView) && !child.equals(mFooterViewContainer)) {
                    mTarget = child;
                    break;
                }
            }
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        mFooterViewContainer.layout((width / 2 - footViewWidth / 2), bottom - footViewHeight
                , (width / 2 + footViewWidth / 2), bottom);
    }
                ...
                if (canChildScrollUp()) {
                    if (mTarget instanceof AbsListView) {
                        ...
                        if (lastPosition + 1 == absListView.getCount()) {
                            if (mCanLoadMore && null != mLoadMoreListener) {
                                loadingView.getProgress().start();
                        ...
                        LinearLayoutManager lLManager = (LinearLayoutManager) recyclerView
                                .getLayoutManager();
                        if ((lLManager.findLastVisibleItemPosition() + 1) == lLManager
                                .getItemCount()) {
                            if (mCanLoadMore && null != mLoadMoreListener) {
                                loadingView.getProgress().start();
                                mFooterViewContainer.setVisibility(View.VISIBLE);
    ...
    public void hideLoadMore() {
        loadingView.getProgress().stop();
        mFooterViewContainer.setVisibility(View.GONE);
    }

    private float getMotionEventY(MotionEvent ev, int activePointerId) {
        if (activePointerId >= MotionEventCompat.getPointerCount(ev)) {
            Log.e(LOG_TAG,
                    "pointerIndex >= MotionEventCompat.getPointerCount(ev)");
            return -1;
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged && mCanRefresh) {
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
                        mActivePointerId);
                if (pointerIndex >= MotionEventCompat.getPointerCount(ev)) {
                    Log.e(LOG_TAG,
                            "pointerIndex >= MotionEventCompat.getPointerCount(ev)");
                    return false;
                }
    ...
    private void onSecondaryPointerUp(MotionEvent ev) {
        ...
        if (pointerId == mActivePointerId) {
            ...
            if (newPointerIndex >= MotionEventCompat.getPointerCount(ev)) {
                Log.e(LOG_TAG,
                        "pointerIndex >= MotionEventCompat.getPointerCount(ev)");
                return;
            }
    ...
    /**
     * @author zfeng Classes that wish to be notified when the swipe gesture
     * correctly triggers a load should implement this interface.
     */
